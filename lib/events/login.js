...
export default class loginEvent extends EventListener {
  ...
  async slider (event) {
    ...
    console.log('\n---------------------------------------------------------')
    console.log('采用项目：https://github.com/mzdluo123/TxCaptchaHelper')
    console.log('本服务需要APP支持，APP下载地址在上方Github的readme.md里')
    console.log(`请复制上面链接，把ssl.captcha.qq.com换成${logger.red('txhelper.glitch.me')}，根据提示操作，待完成后${logger.red('显示')}Ticket时复制到下面。\n`)
